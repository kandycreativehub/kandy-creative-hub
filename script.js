...
function openVideoModal(videoSrc, videoType, title, category) {
    ...
    if (modalVideoCategory) modalVideoCategory.textContent = category || 'AI Video Production';
    ...
    } else if (videoType === 'drive' || videoType === 'iframe') {
        if (modalHtml5Video) {
            modalHtml5Video.pause();
            modalHtml5Video.classList.add('hidden');
        }
...
document.addEventListener('click', (e) => {
    if (e.target.closest('#closeVideoModal') || e.target.id === 'closeVideoModal') {
        closeVideoModal();
    } else if (e.target.id === 'videoModal' || e.target.id === 'videoModalBackdrop') {
        closeVideoModal();
    }
});
